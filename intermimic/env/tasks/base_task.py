...
class BaseTask():
    ...
    def create_viewer(self): # 여기
        ...
        if self.headless == False:
            ...
            self.gym.subscribe_viewer_keyboard_event(self.viewer, gymapi.KEY_S, "toggle_viewer_sync")
            ...
            self.gym.subscribe_viewer_keyboard_event(self.viewer, gymapi.KEY_V, "joint_visualize")
        ...
        if self.viewer:
            ...
            for evt in self.gym.query_viewer_action_events(self.viewer):
                ...
                if evt.action == "PAUSE" and evt.value > 0:
                    ...
                    if self.paused:
                        # Print current frame information when paused
                        if hasattr(self, 'progress_buf'):
                            current_frame = self.progress_buf[0].item()
                            print(f"\n{'='*50}")
                            print(f"PAUSED - Environment {0}")
                            print(f"Current Frame: {int(current_frame)}")
                            if hasattr(self, 'max_episode_length'):
                                print(f"Max Episode Length: {self.max_episode_length}")
                                progress_percent = (current_frame / self.max_episode_length) * 100
                                print(f"Progress: {progress_percent.item():.1f}%")
                            print(f"{'='*50}\n")
                    else: 
                        print("RESUMED")
                elif evt.action == "CONTINUE" and evt.value > 0:
                    # C key: execute one step when paused
                    if self.paused:
                        self.step_once = True
                        if hasattr(self, 'progress_buf'):
                            current_frame = self.progress_buf[0].item()
                            print(f"→ Step forward to frame {int(current_frame + 1)}/{self.max_episode_length.item()}")
                        else:
                            print("→ Step forward 1 frame")
                elif evt.action == "joint_visualize" and evt.value > 0:
    ...
    def _physics_step(self):
        ...
        for i in range(self.control_freq_inv):
            ...
            if not self.paused and self.enable_viewer_sync:
                # Normal execution when not paused
                should_simulate = True
            elif self.step_once:
                # Execute one step when C key pressed during pause
                should_simulate = True
                self.step_once = False  # Reset flag after executing
