...
class InterMimic_MULTI_OBJ(Humanoid_MULTI_OBJ_SMPLX):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self._state_init = InterMimic_MULTI_OBJ.StateInit[state_init]
        ...
        # Option 1: Integer - single object index (e.g., 0 = only object 0)
        # Option 2: Boolean - True = only first object, False = all objects
        # Option 3: List of indices - [0, 2] = objects at index 0 and 2 collide with human
        # Option 4: "all" or None - all objects collide with human (default)
        collision_config = cfg['env'].get('humanObjectCollision', 'all')
        if isinstance(collision_config, int):
            # Single integer: only that object index collides with human
            self._human_collision_objects = [collision_config]
        elif isinstance(collision_config, bool):
            # Backward compatibility: True = only first object, False = all objects
            self._human_collision_objects = [0] if collision_config else None
        elif isinstance(collision_config, list):
            # List of object indices that should collide with human
            self._human_collision_objects = collision_config
        else:
            # Default: 'all' or any other value = all objects collide
            self._human_collision_objects = None
        ...
        self.object_parts = {'kettle': ['base'], 'cup': ['base'], 'diningtable': ['base'], 
        'refrigerator': ['base', 'part1', 'part2'], 'pot': ['base'], 'cuttingboard': ['base'], 'bookshelf': ['base'], 
        'trashbin': ['base', 'part1'], 'gasstove': ['base', 'part1', 'part2'], 'washingmachine': ['base', 'part1'], 
        'microwave': ['base', 'part1'], 'sink': ['base', 'part1', 'part2'], 'desk': ['base'], 'chair': ['base'], 
        'laptop': ['base', 'part1'], 'potlid': ['base'], 'drawer': ['base', 'part1', 'part2'], 'pan': ['base'], 
        'knife': ['base'], 'salt': ['base'], 'book': ['base']}

        # # for every motion files, not for single
        self.reward_object_name = [motion_example.split('_')[-2].split(',')[0] for motion_example in self.motion_file] # ['kettle', 'laptop', 'desk', 'desk', 'desk', 'chair', 'chair']
        self.spawn_objects_name = [motion_example.split('_')[-2].split(',') for motion_example in self.motion_file] # [['kettle', 'diningtable', 'desk'], ['laptop', 'chair', 'desk'], ...]
        self.all_spawn_objects_name = list(set(sum([spawn_objs for spawn_objs in self.spawn_objects_name], []))) # For whole dataset
        self.objects_part_name = [[self.object_parts[obj_name] for obj_name in motion_example.split('_')[-2].split(',')] for motion_example in self.motion_file] # [[['base', 'part1'], ['base', 'part1', 'part2']], [['base', 'part1'], ['base', 'part1', 'part2'], ['base', 'part1', 'part2']], ...]
        self.objects_part_name = [[part_name for sublist in object_part_names for part_name in sublist] for object_part_names in self.objects_part_name] # [['base', 'part1', 'base', 'part1', 'part2', 'base', 'part1', 'part2'], ['base', 'part1', 'base', 'part1', 'part2', 'base', 'part1', 'part2', 'base', 'part1', 'part2'], ...]
        self.reward_object_part_idx = [motion_example.split('_')[-1].split('.')[0].index('1') for motion_example in self.motion_file]
        self.reward_object_part = [self.objects_part_name[idx][self.reward_object_part_idx[idx]] for idx in range(len(self.reward_object_part_idx))] # ['part1', 'part1', 'part2', 'part2', 'part2', 'part1', 'part1']
        reward_object_name_set = sorted(list(set(self.reward_object_name)))

        # Determine max number of objects across all scenes for fixed-size tensors
        self.max_objects = max(len(objs) for objs in self.spawn_objects_name)
        print(f"[Multi-Object] Max objects in any scene: {self.max_objects}")

        # object code - build unified dictionary without overlap
        # Each motion file contains object_code dict: {obj_name: (1, 512)}
        ...
        # Total observation size including multi-object data
        # Multi-object section: (max_objects - 1) * 222 (obj1, obj2, ... excluding obj0)
        # obj0 is already in the base 1211 dims
        # Total: 1211 + (max_objects - 1) * 222
        # Example: max_objects=3 → 1211 + 2*222 = 1655
        self.ref_hoi_obs_size = self.base_hoi_obs_size + (self.max_objects - 1) * 222

        self.num_motions = len(self.motion_file)
        self.dataset_index = to_torch([int(data_path.split('_')[-4].split('/')[-1][1:]) for data_path in self.motion_file], dtype=torch.long).cuda()
        ...
        for i in range(len(self.reward_object_name)):
            self.object_points_new[i] = self.object_points[self.reward_object_name[i]+"_base"]
        ...
        # Weighting network observation buffer
        self.weighting_obs_buf = torch.zeros((self.num_envs, self._get_weighting_obs_size()),
    ...
    def _load_motion(self, motion_file, startk=0, topk=1, initk=0):
        ...
        for idx, data_path in enumerate(motion_file):
            ...
            loaded_dict = {}
            objects = torch.load(data_path)['objects']
            joints = torch.load(data_path)['joints']

            reward_object = f"{self.reward_object_name[self.reward_object_id[idx].item()]}_{self.reward_object_part[idx]}"

            spawn_objects = self.spawn_objects_name[idx] # without base, part1, part2, ... such as ['kettle', 'diningtable', 'desk', 'laptop']
            spawn_objects_idxs = [objects.index(f'{spawn_object}_base') for spawn_object in spawn_objects]

            n_objects = len(objects)

            hoi_data = torch.load(data_path)['data'][startk:]
            ...
            loaded_dict['body_rot'] = loaded_dict['hoi_data'][:, 162+52*3:162+52*3+52*4].clone() # [frame, 208]
            ...
            for i in range(n_objects):
                loaded_dict[f'obj{i}_pos'] = loaded_dict['hoi_data'][:, 526+i*65:529+i*65].clone() # [frame, 3]
                loaded_dict[f'obj{i}_pos_vel'] = (loaded_dict[f'obj{i}_pos'][1:,:].clone() - loaded_dict[f'obj{i}_pos'][:-1,:].clone())*self.fps_data # [frame-1, 3]
                if self.init_vel:
                    loaded_dict[f'obj{i}_pos_vel'] = torch.cat((loaded_dict[f'obj{i}_pos_vel'][:1],loaded_dict[f'obj{i}_pos_vel']),dim=0) # [frame, 3] -> initial velocity same as frame 2 velocity
                else:
                    loaded_dict[f'obj{i}_pos_vel'] = torch.cat((torch.zeros((1, loaded_dict[f'obj{i}_pos_vel'].shape[-1])).to('cuda'),loaded_dict[f'obj{i}_pos_vel']),dim=0) # [frame, 3]

                loaded_dict[f'obj{i}_rot'] = loaded_dict['hoi_data'][:, 529+i*65:533+i*65].clone() # [frame, 4]
                obj_rot_exp_map = torch_utils.quat_to_exp_map(loaded_dict[f'obj{i}_rot']) # [frame, 3]
                loaded_dict[f'obj{i}_rot_vel'] = (obj_rot_exp_map[1:,:].clone() - obj_rot_exp_map[:-1,:].clone())*self.fps_data # [frame-1, 3]
                loaded_dict[f'obj{i}_rot_vel'] = torch.cat((torch.zeros((1, loaded_dict[f'obj{i}_rot_vel'].shape[-1])).to('cuda'),loaded_dict[f'obj{i}_rot_vel']),dim=0) # [frame, 3]

                # Process contact data for ALL objects (not just reward object)
                # This is needed for multi-object weighting network
                ...
                human_contact_raw = loaded_dict['hoi_data'][:, 539+i*65:591+i*65].clone()

                # Only apply contact filtering for reward object (for backward compatibility)
                if objects[i] == reward_object:
                    loaded_dict['hoi_data'][:, 539+i*65:591+i*65] = torch.where(
                        human_contact_raw == -1,
                ...
                obj_contact_raw = loaded_dict['hoi_data'][:, 538+i*65:539+i*65].clone()

                # Only apply contact filtering for reward object (for backward compatibility)
                if objects[i] == reward_object:
                    loaded_dict['hoi_data'][:, 538+i*65:539+i*65] = torch.where(
                ...
                # Store contact data for this object (needed for all objects now)
                loaded_dict[f'obj{i}_contact_obj'] = obj_contact_raw.clone()
                loaded_dict[f'obj{i}_contact_human'] = human_contact_raw.clone()

                # Compute interaction graph (IG) for this object
                # [frame, 1, 4] ->[frame, 1024, 4] -> [frame*1024, 4]
                obj_rot_extend = loaded_dict[f'obj{i}_rot'].unsqueeze(1).repeat(1, self.object_points[objects[i]].shape[0], 1).view(-1, 4)
                # [1, 1024, 3] -> [frame, 1024, 3] -> [frame*1024, 3]
                object_points_extend = self.object_points[objects[i]].unsqueeze(0).repeat(loaded_dict[f'obj{i}_rot'].shape[0], 1, 1).view(-1, 3)
                obj_points = torch_utils.quat_rotate(obj_rot_extend, object_points_extend).view(loaded_dict[f'obj{i}_rot'].shape[0], self.object_points[objects[i]].shape[0], 3) + loaded_dict[f'obj{i}_pos'].unsqueeze(1)

                # frame, 52, 3 -> frame*52, 3
                ...
                ref_ig = quat_rotate(heading_rot_extend, ref_ig).view(loaded_dict[f'obj{i}_rot'].shape[0], -1) # frame, 156
                loaded_dict[f'obj{i}_ig'] = ref_ig
                if '_base' in objects[i]:
                    if objects[i].replace('_base', '') in spawn_objects and objects[i].replace('_base', '')+'_part1' in objects:
                        loaded_dict[f'obj{i}_part1_joint'] = joints[objects[i].replace('_base', '')+'_part1']['states']
                    if objects[i].replace('_base', '') in spawn_objects and objects[i].replace('_base', '')+'_part2' in objects:
                        loaded_dict[f'obj{i}_part2_joint'] = joints[objects[i].replace('_base', '')+'_part2']['states']
            ...
            loaded_dict['hoi_data'] = torch.cat((
                ...
                loaded_dict[f'obj0_pos'].clone(), # f, 3
                loaded_dict[f'obj0_rot'].clone(), # f, 4
                loaded_dict[f'obj0_pos_vel'].clone(), # f, 3
                loaded_dict[f'obj0_rot_vel'].clone(), # f, 3
                loaded_dict[f'obj0_ig'].clone(), # f, 156
                loaded_dict[f'obj0_contact_human'].clone(), # f, 52
                loaded_dict[f'obj0_contact_obj'].clone(), # f, 1
                ), dim=-1) # f, 1211

            # Multi-object data for weighting network (fixed size: max_objects - 1)
            # obj0 is already in hoi_data above, so only add obj1, obj2, ...
            # Each object: pos(3) + rot(4) + pos_vel(3) + rot_vel(3) + ig(156) + contact_human(52) + contact_obj(1) = 222 dims
            multi_obj_components = []
            for obj_idx in range(1, self.max_objects):  # Start from 1, not 0!
                if obj_idx in spawn_objects_idxs:
                    # Object exists in this scene
                    multi_obj_components.extend([
                        loaded_dict[f'obj{obj_idx}_pos'].clone(),           # f, 3
                        loaded_dict[f'obj{obj_idx}_rot'].clone(),           # f, 4
                        loaded_dict[f'obj{obj_idx}_pos_vel'].clone(),       # f, 3
                        loaded_dict[f'obj{obj_idx}_rot_vel'].clone(),       # f, 3
                        loaded_dict[f'obj{obj_idx}_ig'].clone(),            # f, 156
                        loaded_dict[f'obj{obj_idx}_contact_human'].clone(), # f, 52
                        loaded_dict[f'obj{obj_idx}_contact_obj'].clone(),   # f, 1
                    ])
                else:
                    # Object doesn't exist - pad with zeros (222 dims per object)
                    num_frames = loaded_dict['root_pos'].shape[0]
                    multi_obj_components.extend([
                        torch.zeros((num_frames, 3), device='cuda'),   # pos
                        torch.zeros((num_frames, 4), device='cuda'),   # rot (could use identity quat [0,0,0,1])
                        torch.zeros((num_frames, 3), device='cuda'),   # pos_vel
                        torch.zeros((num_frames, 3), device='cuda'),   # rot_vel
                        torch.zeros((num_frames, 156), device='cuda'), # ig
                        torch.zeros((num_frames, 52), device='cuda'),  # contact_human
                        torch.zeros((num_frames, 1), device='cuda'),   # contact_obj
                    ])

            # Append multi-object data to hoi_data
            loaded_dict['hoi_data'] = torch.cat([
                loaded_dict['hoi_data'],  # f, 1211 (original, includes obj0)
                *multi_obj_components      # f, (max_objects - 1) * 222 (obj1, obj2, ...)
            ], dim=-1)  # f, 1211 + (max_objects - 1) * 222
            ...
            hoi_ref = torch.cat((
                ...
                loaded_dict['dof_pos'].clone(), # f, 153
                loaded_dict['dof_vel'].clone(),), dim=-1) # f, 153
                # f, 319
            # 3 objects (currently no joints)
            obj_ref = torch.cat([
                *[
                    torch.cat([
                        loaded_dict[f'obj{i}_pos'].clone(),
                        loaded_dict[f'obj{i}_pos_vel'].clone(),
                        loaded_dict[f'obj{i}_rot'].clone(),
                        loaded_dict[f'obj{i}_rot_vel'].clone()
                    ], dim=-1)
                    for i in spawn_objects_idxs
                ]
            ], dim=-1) # f, 39


            hoi_ref = torch.cat((hoi_ref, obj_ref), dim=-1) # f, 358
        ...
        # pad sequence length for each motion
        for i, (data, ref) in enumerate(zip(hoi_datas, hoi_refs)):
            ...
            self.hoi_refs.append(F.pad(ref, hoi_ref_pad_size, "constant", 0))
    ...
    def create_component_stat(self, loaded_dict):
        ...
        data_component_order_idx = [
            'root_pos', 'root_rot', 'dof_pos', 'dof_vel', 'body_pos', 'body_rot', 'body_pos_vel', 'body_rot_vel',
            f'obj0_pos', f'obj0_rot', f'obj0_pos_vel', f'obj0_rot_vel',
            f'obj0_ig', f'obj0_contact_human', f'obj0_contact_obj'
        ]
        ...
        for obj_idx in range(3):
            self.ref_component_order.extend([f'obj{obj_idx}_pos', f'obj{obj_idx}_pos_vel', f'obj{obj_idx}_rot', f'obj{obj_idx}_rot_vel'])
    ...
    def extract_ref_component(self, var_name, data_id, ref_index, t):
        if var_name[:3] == 'obj':
            indices = [self.ref_component_order.index(var) for var in [f"{var_name.split('_')[0]}{i}{var_name[3:]}" for i in range(3)]]
            starts = [self.ref_component_index[index] for index in indices]
            ends = [self.ref_component_index[index + 1] for index in indices]
            return torch.stack([self.hoi_refs[data_id, ref_index, t, starts[i]:ends[i]] for i in range(3)], dim=1)
    ...
    def _load_target_asset(self):
        ...
        asset_root = "intermimic/data/assets/parahome/sim_object"
        self._target_asset = {}
        self.object_points = {}
        self.object_normals = {}
        ...
        for obj_name in self.all_spawn_objects_name:
            ...
            if obj_name in ['kettle', 'cup', 'bookshelf']:
                asset_options.vhacd_params.max_convex_hulls = 80
            elif obj_name in ['chair']:
                asset_options.vhacd_params.max_convex_hulls = 50

            # Set fixed base for large furniture objects
            if obj_name in ['refrigerator', 'washingmachine', 'microwave', 'gasstove',
                            'sink', 'drawer', 'desk', 'diningtable', 'bookshelf']:
                asset_options.fix_base_link = True

            self._target_asset[obj_name] = self.gym.load_asset(self.sim, asset_root, asset_file, asset_options)

            # Load object points and normals for ALL object parts (not just reward objects)
            # This is needed for computing IG (interaction graph) for multi-object weighting
            for part in self.object_parts[obj_name]:
                obj_file = f"{asset_root}/{obj_name}/{part}_collision.obj"

                # Check if file exists
                import os
                if not os.path.exists(obj_file):
                    print(f"⚠️ Warning: Collision mesh not found: {obj_file}, skipping...")
                    continue
                ...
                # Use face normals at the sampled points
    ...
    def _build_target_tensors(self):
        ...
        # [num_envs, 3, 13]
        self._target_states = self._root_states.view(self.num_envs, num_actors, self._root_states.shape[-1])[..., 1:4, :]
        
        self._tar_actor_ids = to_torch(num_actors * np.arange(self.num_envs), device=self.device, dtype=torch.int32) + 1
        offsets = torch.tensor([0, 1, 2]).to(self.device)
        self._tar_actor_ids = (self._tar_actor_ids[:, None] + offsets)
        
        bodies_per_env = self._rigid_body_state.shape[0] // self.num_envs # 55 = 52 + 3objs
        contact_force_tensor = self.gym.acquire_net_contact_force_tensor(self.sim)
        contact_force_tensor = gymtorch.wrap_tensor(contact_force_tensor)
        self._tar_contact_forces = contact_force_tensor.view(self.num_envs, bodies_per_env, 3)[..., self.num_bodies, :] # 1st obj
        return
    ...
    def _reset_actors(self, env_ids):
        # human reset part
        if (self._state_init == InterMimic_MULTI_OBJ.StateInit.Default):
            self._reset_default(env_ids)
        elif (self._state_init == InterMimic_MULTI_OBJ.StateInit.Start
              or self._state_init == InterMimic_MULTI_OBJ.StateInit.Random):
            self._reset_ref_state_init(env_ids)
        elif (self._state_init == InterMimic_MULTI_OBJ.StateInit.Hybrid):
    ...
    def _reset_ref_state_init(self, env_ids):
        ...
        if (self._state_init == InterMimic_MULTI_OBJ.StateInit.Random
            or self._state_init == InterMimic_MULTI_OBJ.StateInit.Hybrid):
    ...
    def build_hoi_observations(self, root_pos, root_rot, root_vel, root_ang_vel, dof_pos, dof_vel, body_pos, 
        ...
        # ============ human body & obj contact indicator==============
        ...
        obs_base = torch.cat((
            ...
            dof_pos,   # 2048, 153
            dof_vel,   # 2048, 153
            body_pos.reshape(body_pos.shape[0],-1), # 2048, 156
            body_rot.reshape(body_rot.shape[0],-1), # 2048, 208
            body_vel.reshape(body_vel.shape[0],-1), # 2048, 208
            body_rot_vel.reshape(body_rot_vel.shape[0],-1), # 2048, 156
            reward_target_states[:,0,:],    # 2048, 13
            ig,                             # 2048, 156
            contact,                        # 2048, 52
            target_contact                  # 2048, 1
            ), dim=-1) # 2048, 1211
    ...
    def _compute_reward(self, env_ids=None):
        ...
        self._curr_reward[env_ids, self.progress_buf - self.start_times] = self.rew_buf
        self._sum_reward[env_ids] += self.rew_buf
        self._curr_state[env_ids, self.progress_buf - self.start_times, :] = torch.cat([
    ...
    def compute_obj_reward(self, w):
        ...
        # Store L2 distance (meters) for evaluation
        self.extras['obj_pos_error'] = torch.sqrt(torch.sum((ref_local_obj_pos - local_obj_pos)**2, dim=-1))
        ...
        # Store rotation error (radians) for evaluation
        self.extras['obj_rot_error'] = eor
    ...
    def compute_cg_reward(self, w):
        contact_thres = 0.1
        ref_human_contact = self.extract_data_component('contact_human', obs=self._curr_ref_obs) # (2048, 52)
        human_contact = self.extract_data_component('contact_human', obs=self._curr_obs) # (2048, 52)
        ...
        # ecg_left = (((ref_left_contact_hand_any > contact_thres) * torch.abs(left_hand_contact - ref_left_contact_hand_any)).mean(dim=-1))
        ...
        # ecg_right = (((ref_right_contact_hand_any > contact_thres) * torch.abs(right_hand_contact - ref_right_contact_hand_any)).mean(dim=-1))
