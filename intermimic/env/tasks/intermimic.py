...
class InterMimic(Humanoid_SMPLX):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self.object_name = [motion_example.split('_')[-2] for motion_example in self.motion_file] # [chair, chair, chair, desk, desk, door, door]

        object_name_set = sorted(list(set(self.object_name))) # ['chair', 'desk', 'door']
        self.object_id = to_torch([object_name_set.index(name) for name in self.object_name], dtype=torch.long).cuda() # [0, 0, 0, 1, 1, 2, 2]
        # [[T, T, T, F, F, F, F], [F, F, F, T, T, F, F], [F, F, F, F, F, T, T]]
        self.obj2motion = torch.stack([self.object_id == k for k in range(len(object_name_set))], dim=0)
        self.object_name = object_name_set # ['chair', 'desk', 'door']
    ...
    def _load_target_asset(self): # smplx
        ...
        for i, object_name in enumerate(self.object_name):
            ...
            object_points, object_faces = trimesh.sample.sample_surface_even(mesh_obj, count=1024, seed=1024)
    ...
    def _build_target(self, env_id, env_ptr):
        ...
        target_handle = self.gym.create_actor(
            env_ptr, 
            self._target_asset[env_id % len(self.object_name)], 
            default_pose, 
            self.object_name[env_id % len(self.object_name)], 
            col_group, 
            col_filter, 
            segmentation_id)
    ...
    def _compute_observations(self, env_ids=None):
        if (env_ids is None):
            ...
            # DEBUG: Print observation components
            print(f"🌍 [ENVIRONMENT] OBSERVATION T=1 SHAPE: {obs_t1.shape}")  
            print(f"🌍 [ENVIRONMENT] OBSERVATION T=16 SHAPE: {obs_t16.shape}")
            print(f"🌍 [ENVIRONMENT] FINAL OBS SHAPE: {self.obs_buf.shape}")
            print(f"🌍 [ENVIRONMENT] CURRENT FRAME: {self.progress_buf[0].item()}")
            ...
            self.obs_buf[env_ids] = torch.cat((obs_t1, obs_t16), dim=-1)
    ...
    def _compute_reward(self, actions):
        ...
        # DEBUG: Print reward components
        print(f"🏆 [REWARDS] Human: {rb.mean():.4f}, Object: {ro.mean():.4f}, IG: {rig.mean():.4f}, Contact: {rcg.mean():.4f}", f"Total: {self.rew_buf.mean():.4f}")
        
        wandb.log({
            "rewards/human": rb.mean().item(),
            "rewards/object": ro.mean().item(), 
            "rewards/ig": rig.mean().item(),
            "rewards/contact": rcg.mean().item(),
            "rewards/total": self.rew_buf.mean().item(),
            "step": self.common_step_counter if hasattr(self, 'common_step_counter') else 0
        })
