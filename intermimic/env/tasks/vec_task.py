...
class VecTaskPython(VecTask):
    ...
    def step(self, actions):
        ...
        # Check if weighting observation exists
        if hasattr(self.task, 'weighting_obs_buf'):
            weighting_obs = torch.clamp(self.task.weighting_obs_buf, -self.clip_obs, self.clip_obs).to(self.rl_device)
            obs_dict = {
                'obs': obs,
                'weighting_obs': weighting_obs
            }
